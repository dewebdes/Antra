...
async function submitBuyOffer({ coinName, price, money, profit }) {
    ...
    try {
        ...
        //const stopId = await putBuyOrder(coinName, money, price, cid);
...
async function processBuyOrder(order) {
    const { market, amount, filled_amount, price, client_id, base_fee } = order;

    console.log(JSON.stringify(order));

    //order.filled_amount = 
    var filled_amount2 = Number(filled_amount) - Number(base_fee);
    ...
    const sellOrderId = await putSellOrder(coin, filled_amount2, sellPrice, sellCid);
...
async function putSellOrder_old(coin, amount, limit, cid) {
    console.log("sell : " + coin + " - " + amount + " - " + limit);
    const timestamp = Date.now();
    const data = JSON.stringify({
        market: `${coin}USDT`,
        market_type: "SPOT",
...
async function putSellOrder(coin, amount, limit, cid) {
    ...
    const data = JSON.stringify({
        ...
        market_type: "SPOT",
        side: "sell",
        type: "market",             // Change from "limit" to "market" for stop-market execution
        amount: amount,             // Quantity of base asset to sell
        trigger_price: limit,       // Use 'limit' parameter as trigger price
        client_id: cid,
        is_hide: false              // Optional: set to true to hide the order pre-trigger
    });

    console.log(`Submitting stop-market SELL order with CID: ${cid}`);
    ...
    try {
        ...
        if (res.data.code === 0 && res.data.data.stop_id) {
            console.log(`Successfully placed stop-market SELL order. Stop ID: ${res.data.data.stop_id}`);
            return res.data.data.stop_id;
        } else {
            throw new Error(`Unexpected response: ${JSON.stringify(res.data)}`);
        }
    } catch (error) {
        console.error("Error placing stop-market SELL order:", error.message);
        throw error;
    }
}
...
async function fetchFinishedOrders() {
    const timestamp = Date.now();
    const requestPath = "/v2/spot/finished-order?market_type=SPOT&limit=100";
...
async function putBuyOrder(coin, amount, limit, cid) {
    console.log("sell : " + coin + " - " + amount + " - " + limit);
    const coinQty = (amount / limit).toFixed(6);
...
async function putMarketOrder0(coin, money, limit, side, cid) {
    const timestamp = Date.now();
    const data = JSON.stringify({
        market: `${coin}USDT`,
        market_type: "SPOT",
...
// 🔧 Submit stop-limit buy/sell order
async function putMarketOrder(coin, money, limit, side, cid) {
    const timestamp = Date.now();

    const triggerPrice = applyTriggerDelta(limit, 1.0133);             // +1.33% trigger price
    const amount = calculateAmount(money, limit, getDecimalPrecision(limit)); // base asset quantity

    const data = JSON.stringify({
        market: `${coin}USDT`,
        market_type: "SPOT",
        side: side,
        type: "limit",               // Stop-limit order type
        amount: amount,              // Base asset quantity
        price: limit,                // Execution price
        trigger_price: triggerPrice, // Trigger activation price
        client_id: cid,
        is_hide: false               // Optional: hide until triggered
        // ccy is omitted since it's non-essential
    });

    console.log(`Submitting stop-limit ${side} order with CID: ${cid}`);
    const authHeader = createAuthorization("POST", "/v2/spot/stop-order", data, timestamp);
    ...
    try {
        ...
        if (res.data.code === 0 && res.data.data.stop_id) {
            console.log(`Stop-limit order placed successfully: Stop ID ${res.data.data.stop_id}`);
            return res.data.data.stop_id;
        } else {
            throw new Error(`Unexpected response: ${JSON.stringify(res.data)}`);
        }
    } catch (error) {
        console.error(`Error placing stop-limit ${side} order:`, error.message);
        throw error;
    }
}

// 🔍 Calculate trigger price with adjusted delta (e.g. +1.33%)
function applyTriggerDelta(basePrice, multiplier = 1.0133) {
    const precision = getDecimalPrecision(basePrice);
    return Number((basePrice * multiplier).toFixed(precision));
}

// 📐 Determine decimal precision from a price value
function getDecimalPrecision(value) {
    const decimals = value.toString().split('.')[1];
    return decimals ? decimals.length : 0;
}

// 🧮 Calculate base asset amount using quote money and price
function calculateAmount(money, price, precision = 6) {
    return Number((money / price).toFixed(precision));
}
